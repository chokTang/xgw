 ...
 * @date :2019/7/16 9:36
 */
class SupplierAccountViewModel : BaseViewModel() {
    ...
    fun loadMore() {
        ...
        service.getAccountRecord(map).netDispatch(object : RxNetObserver<ListBean<MoneyRecordBean>>() {
            ...
            override fun onSuccess(t: ListBean<MoneyRecordBean>?) {
                mPage++
                t?.run {
                    this@SupplierAccountViewModel .list.addAll(list)
                }
                changeLoadType(this@SupplierAccountViewModel.list, t?.list, false)
            }
        })
    }
    ...
    fun goPayDetail(){
        startRout(RoutUrl.Supplier.payment_detail)
    }
}